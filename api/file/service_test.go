...
func TestFileService(t *testing.T) {
	Describe("File: Table Columns From Rows", func() {
		BeforeEach(func() {
...
func TestSaveFileAsDatasetHandler(t *testing.T) {
	...
	if assert.NoError(t, fs.saveFileAsDatasetHandler(c), "saveFileAsDatasetHandler Failing") {
		...
		assert.Equal(t, 3, len(resBody.Attributes.Tables))
		assert.Equal(t, 4, len(resBody.Attributes.Tables[0].Columns))
		assert.Equal(t, 10, len(resBody.Attributes.Tables[0].Columns["92"]))
		assert.Equal(t, "60", resBody.Attributes.Tables[0].Columns["92"][0])
...
func TestGetDatasetsHandler(t *testing.T) {
	...
	if assert.NoError(t, fs.getDatasetsHandler(c), "getDatasetsHandler Failing") {
...
func TestUpdateDatasetHandler(t *testing.T) {
	...
	if assert.NoError(t, fs.updateDatasetHandler(c), "getDatasetsHandler Failing") {
		resBody, expected_res := new(Dataset), new(Dataset)
...
func TestGetDatasetByIdHandler(t *testing.T) {
	...
	if assert.NoError(t, fs.getDatasetByIdHandler(c), "saveFileAsDatasetHandler Failing") {
		resBody, expected_res := new(Dataset), new(Dataset)
...
func TestGetDatasetWithInvalidId(t *testing.T) {
	...
	err := fs.getDatasetByIdHandler(c)
...
func TestDeleteDatasetHandler(t *testing.T) {
	...
	if assert.NoError(t, fs.deleteDatasetHandler(c), "getDatasetsHandler Failing") {
